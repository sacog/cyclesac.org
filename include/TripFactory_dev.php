...
class TripFactory
{
	...
	public static function insert( $user_id, $purpose, $comfort, $notes, $start )
	{
		...
		if ( ( $db->query( $query ) === true ) &&
			 ( $id = $db->insert_id ) )
		{
			Util::log( __METHOD__ . "() created new trip {$id} for user {$user_id}, start {$start}, {$purpose}, {$comfort}: {$notes}" );
			return self::getTrip( $id );
		}
		else
			Util::log( __METHOD__ . "() ERROR failed to create new trip for user {$user_id}, start {$start}, {$purpose}, {$comfort}: {$notes}" );
